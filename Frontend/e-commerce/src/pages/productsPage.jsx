...
const ProductsPage = () => {
  ...
  const [sortOrder, setSortOrder] = useState(-1); // Descending by default
      ...
      <div style={{ flex: 3 }}>
        ...
        {products.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' }}>
            {products.map(prod => (
              <div key={prod._id} style={{ border: '1px solid #ccc', padding: '10px', borderRadius: '5px' }}>
                <h4>{prod.name}</h4>
                <p>₹{prod.price}</p>
                <p>{prod.category}</p>
              </div>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div>
          ...
          <button
            ...
            style={{ marginTop: '10px', background: '#ccc', border: 'none', padding: '6px 12px', cursor: 'pointer' }}
          >
            Clear Category
          </button>
        </div>
